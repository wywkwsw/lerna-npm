(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('tiny-pinyin'), require('vue')) :
    typeof define === 'function' && define.amd ? define(['exports', 'tiny-pinyin', 'vue'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.MyBundle = {}, global.pinyin, global.Vue));
}(this, (function (exports, pinyin, Vue) { 'use strict';
    ...
    const ieVersion = Vue__default['default'].prototype.$isServer ? 0 : Number(document.documentMode);
    ...
    //防抖
    function debounce(fn, who, delay) {
        var timeoutID = null;
        return function () {
    ...
    //模糊搜索
    function fuzzySearch(total, library, searchValue, typeOne, typeTwo, typeThree) {
    ...
    //数组中对象根据某个属性去重
    function reduce(arr, property) {
        var hash = {};
        arr = arr.reduce(function (item, next) {
            hash[next[property]] ? '' : hash[next[property]] = item.push(next);
            return item
        }, []);
        return arr
    }
    ...
    function formmateDate(startTime, endTime) {
    ...
    function tyMergeCells(cols) {    //sDate1和sDate2是2006-12-18格式
        //  合并单元格
        let keys = null;
        let mergeCells = [];
    ...
    function ArrayToString(Arr) {    //sDate1和sDate2是2006-12-18格式
        let Str = '';
    ...
    function NameChangeID(mydata, dataname) {
        let data = mydata;
        let selectName = [];
    ...
    //  数据转换，   合并单元格后将缺失信息补全
    function mergeCellsChangedata(mydata, mergeCells) {
        let data = mydata;
            ...
            item.find((ones, col, arrones) => {
                for (let i = 0; i < mergeCell.length; i++) {
                    if (mergeCell[i].col == col) {
                        if (mergeCell[i].row == row) ; else if (row > mergeCell[i].row && row < mergeCell[i].rowspan + mergeCell[i].row) {
    ...
    function nullChangeNub(mydata, tableCol) {
        let data = mydata;
        let thArry = tableCol;
        let typeNumArr = [];
    ...
    * other 其它信息
    * */
    function titleSplicing(type, dateType, objectType, template, other) {
        if (type === "1") {
    ...
    //拼音搜索，包含首字母搜索
    function searchWithPinyin(keywords = "", name = "") {
        ...
        //   return false;
        // }
        let pinyinName = pinyin__default['default'].convertToPinyin(name, ""), headpinyinName = pinyin__default['default'].convertToPinyin(name, "-").split("-").map(item => item.slice(0, 1).toUpperCase()).join("");
    ...
    //合并单元格
    function getRowspanWithTwoKeys(data, key, value, row, index, key1, key2) {
     ...
     * @returns 
     */
    function filterTree(tree = [], validate = () => { }, param = [], arr = [], alias = "children") {
    ...
    exports.NameChangeID = NameChangeID;
    exports.analysisText = analysisText;
    ...
    exports.formmateDate = formmateDate;
    ...
    exports.getRowspanWithTwoKeys = getRowspanWithTwoKeys;
    ...
    exports.mergeCellsChangedata = mergeCellsChangedata;
    exports.nullChangeNub = nullChangeNub;
    exports.param2Obj = param2Obj;
    exports.reduce = reduce;
    exports.searchWithPinyin = searchWithPinyin;
    exports.throttle = throttle;
    exports.throttleTwo = throttleTwo;
    exports.titleSplicing = titleSplicing;
    exports.tyMergeCells = tyMergeCells;
